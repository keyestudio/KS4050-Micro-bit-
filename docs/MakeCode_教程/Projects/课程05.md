...
在本项目实验中，OLED显示屏上会实时显示micro:bit加速度传感器检测到的值，当micro:bit主板的加速度传感器检测到倾斜或轻微摇晃时，micro:bit主板的扬声器会响起，同时microbit5×5LED点阵屏显示警报图案，达到警示提醒的效果。
...
将示例代码传成功下载到micro:bit主板后，OLED显示屏上会实时显示micro:bit加速度计检测到的地震水平值等相关信息。

倾斜或轻微摇晃micro:bit主板，当检测到的加速度强度值小于900 或者 大于1200时，OLED显示字符串和加速度强度值，同时Microbit主板上的5×5LED点阵屏显示图案![Img](./media/ab4.png)，扬声器发出警报声；否则，OLED不显示任何信息，5×5LED点阵屏也不显示图案，扬声器也不发声。
